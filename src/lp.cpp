...
SimplexState simplex(detail::StandardForm& f,std::span<const double> cost,bool phase_one,Run& run) {
    ...
    while (true) {
        ...
        Index leaving=f.basis.size(); double step=infinity; bool uncertain_positive=false;
        for (Index i=0;i<direction.size();++i) if (direction[i]>tol.pivot) {
            ...
            const double tie=1e-13*(1+std::abs(ratio));
            if (leaving==f.basis.size() || ratio<step-tie || (std::abs(ratio-step)<=tie && f.basis[i]<f.basis[leaving])) { step=ratio; leaving=i; }
...
DualCertificate certificate(const Model& original,const detail::StandardForm& f,std::span<const double> dual,
    ...
    for (Index i=0;i<f.origins.size();++i) if (f.origins[i].original) {
        ...
        if (y>=0) c.row_lower[row]+=y; else c.row_upper[row]-=y;
    }
    ...
    for (Index pass=0;pass<64*std::max<Index>(1,original.variables.size());++pass) {
        ...
        for (Index j=0;j<original.variables.size();++j) {
            ...
            const auto col=original.matrix.column(j);
            for (Index k=0;k<col.rows.size();++k) {
                const auto i=col.rows[k]; const double a=col.values[k]; double* multiplier=nullptr;
                if (need_up) multiplier=a>0 ? &c.row_upper[i] : &c.row_lower[i];
                else multiplier=a>0 ? &c.row_lower[i] : &c.row_upper[i];
                if (*multiplier>0) { *multiplier=std::nextafter(*multiplier,infinity); changed=true; break; }
            }
        }
        if (!changed) break;
    }
