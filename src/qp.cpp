...
QpResult solve_qp(const QuadraticModel&p,const SolverOptions&o){
 ...
 r.primal=x;r.objective=objective(p,original_q,x);r.verification.objective=r.objective;auto independent_eq=matvec(c.e,x),independent_ineq=matvec(c.g,x);double primal=0;for(Index i=0;i<independent_eq.size();++i)primal=std::max(primal,std::abs(independent_eq[i]-c.b[i]));for(Index i=0;i<independent_ineq.size();++i)primal=std::max(primal,std::max(0.0,independent_ineq[i]-c.h[i]));r.verification.primal_residual=primal;r.verification.stationarity_residual=norm_inf(last_rd);r.verification.complementarity_residual=last_mu;const double threshold=10*std::max(o.tolerances.primal,o.tolerances.dual);r.verification.passed=std::max({r.verification.primal_residual,r.verification.stationarity_residual,r.verification.complementarity_residual})<=threshold;if(!r.verification.passed){r.status=SolveStatus::numerical_failure;r.verification.violations.push_back("independent KKT residual exceeds tolerance");r.message="Candidate failed KKT verification";}else r.message="Convex QP optimum independently verified by primal feasibility and KKT residuals";return finish();
