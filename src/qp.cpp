...
Direction direction(const Dense&q,const Canon&c,const std::vector<double>&s,const std::vector<double>&z,const std::vector<double>&rd,const std::vector<double>&re,const std::vector<double>&ri,const std::vector<double>&rc,double pivot,Index&factorizations,Index&max_nonzeros){
 const Index n=q.size(),me=c.e.size(),mi=c.g.size(),dimension=n+me;std::vector<std::map<Index,double>> rows(dimension);std::vector<double>rhs(dimension);
 for(Index i=0;i<n;++i)for(Index j=0;j<n;++j)if(q[i][j]!=0)rows[i][j]=q[i][j];
 for(Index k=0;k<mi;++k){const double d=z[k]/s[k];for(Index i=0;i<n;++i)if(c.g[k][i]!=0)for(Index j=0;j<n;++j)if(c.g[k][j]!=0)rows[i][j]+=c.g[k][i]*d*c.g[k][j];}
 for(Index i=0;i<n;++i){rhs[i]=-rd[i];for(Index k=0;k<mi;++k)rhs[i]+=c.g[k][i]*(rc[k]-z[k]*ri[k])/s[k];for(Index k=0;k<me;++k)if(c.e[k][i]!=0)rows[i][n+k]=c.e[k][i];}
 for(Index k=0;k<me;++k){rhs[n+k]=-re[k];for(Index j=0;j<n;++j)if(c.e[k][j]!=0)rows[n+k][j]=c.e[k][j];}
 std::vector<Triplet>entries;for(Index i=0;i<dimension;++i)for(const auto&[j,value]:rows[i])if(value!=0)entries.push_back({i,j,value});const auto kkt=CscMatrix::from_triplets(dimension,dimension,std::move(entries));max_nonzeros=std::max(max_nonzeros,kkt.nonzeros());std::vector<Index>columns(dimension);std::iota(columns.begin(),columns.end(),Index{0});SparseBasis factor(kkt,columns,pivot);++factorizations;auto d=factor.solve(rhs);Direction out;out.dx.assign(d.begin(),d.begin()+static_cast<std::ptrdiff_t>(n));out.dy.assign(d.begin()+static_cast<std::ptrdiff_t>(n),d.end());out.ds.resize(mi);out.dz.resize(mi);
