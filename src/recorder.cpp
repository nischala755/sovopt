...
void record_solve(const std::filesystem::path& bundle,const std::filesystem::path& model_source,
    ...
    try {
        write_file(temporary/"manifest.json","{\"format\":\"astraniti-flight-recorder\",\"format_version\":1,\"solver_version\":\"0.1.0\"}\n");
        ...
        std::ostringstream config;config<<std::boolalpha<<std::setprecision(17)<<"{\"iteration_limit\":"<<options.iteration_limit<<",\"node_limit\":"<<options.node_limit<<",\"time_limit_seconds\":"<<options.time_limit_seconds<<",\"mip_gap\":"<<options.mip_gap<<",\"scaling\":"<<options.scaling<<",\"presolve\":"<<options.presolve<<",\"deterministic\":"<<options.deterministic<<",\"cuts\":"<<options.cuts<<",\"rounding\":"<<options.rounding<<",\"branching\":"<<quote(options.branching)<<",\"primal_tolerance\":"<<options.tolerances.primal<<",\"dual_tolerance\":"<<options.tolerances.dual<<",\"integrality_tolerance\":"<<options.tolerances.integrality<<",\"pivot_tolerance\":"<<options.tolerances.pivot<<"}\n";write_file(temporary/"solver_config.json",config.str());
...
ReplayReport replay_solve(const std::filesystem::path& bundle,bool reverify) {
    ...
    try {
        ...
        const auto manifest=read_file(bundle/"manifest.json");if(field_number(manifest,"format_version")!=flight_recorder_format_version){report.message="unsupported recorder format version";return report;}
        ...
        const auto primal=field_array(solution,"primal");const double objective=field_number(solution,"objective");
        ...
        if(report.recorded_status=="optimal") verification=integer?verify_primal(model,primal,objective,{},true):verify_optimality(model,primal,objective,certificate);
        ...
        report.reverification_passed=verification.passed;report.message=verification.passed?"integrity PASS; independent reverification PASS":"integrity PASS; independent reverification FAILED";
